...
const signIn = (email, password) => async (dispatch, getState) => {
  try {
    ...
    const updatedUserCart = await axios.post(
      ...
      { currentCart: productsToCart.products, userCart: data.cart },
      {
        headers: {
    ...
    dispatch({
      type: PRODUCT_TO_CART_UPDATE_AFTER_SIGN_IN,
      payload: updatedUserCart.data,
    });
...
const userUpdateInfo = (info) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post("api/users/update-info", info, {
      headers: {
    ...
    dispatch({
      type: USER_SIGN_IN_AFTER_UPDATE_INFO,
...
const userUpdatePassword = (passwords) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post("api/users/update-password", passwords, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });
...
const userUpdateAvatar = (avatar) => async (dispatch, getState) => {
  try {
    ...
    dispatch({
      type: USER_SIGN_IN_AFTER_UPDATE_AVATAR,
      payload: { ...userInfo, ...data },
    });
    Cookie.set("userInfo", JSON.stringify({ ...userInfo, ...data }));
  } catch (error) {
